...
must_haves:
  ...
  key_links:
    ...
    - from: "src/distill/training/loop.py"
      to: "latent space analysis block"
      via: "Skipped when complex_spectrogram.enabled (incompatible 1-channel analyzer); deferred to Phase 16"
      pattern: "complex_spectrogram\\.enabled.*latent|Skipping latent space analysis"
---
...
6. **Compute 2-channel spectrograms:**
   - Create `ComplexSpectrogram` instance from Plan 01 by passing the `ComplexSpectrogramConfig` directly (it is the sole constructor argument -- single source of truth for STFT params).
...
7. **Skip latent space analysis for 2-channel path:** The existing latent space analysis section at the end of `train()` (around lines 661-705) constructs an `AudioTrainingDataset` (waveform-based) and passes the v1.0 `spectrogram` (1-channel `AudioSpectrogram`) to `analyzer.analyze()`. This is incompatible with a model trained on 2-channel input. When `config.complex_spectrogram.enabled` is True, wrap the latent space analysis block in a conditional skip:
   ```python
   if config.complex_spectrogram.enabled:
       ...
       # existing latent space analysis code (lines 661-705 unchanged)
       ...
   ```
   This prevents the analysis from running with 1-channel spectrograms against a model that expects 2-channel input.
