...
must_haves:
  ...
  key_links:
    ...
    - from: "src/distill/audio/spectrogram.py"
      ...
      via: "ComplexSpectrogramConfig is the sole constructor argument for ComplexSpectrogram (single source of truth for STFT params in v2.0)"
...
**Constructor:**
- Accept a `ComplexSpectrogramConfig` (from `config.py`, Plan 01 Task 2). This is the single source of truth for STFT parameters in v2.0 code paths — the constructor does NOT accept `SpectrogramConfig`. Extract `n_fft`, `hop_length`, `n_mels`, and `if_masking_threshold` from the config.
- Create a `torchaudio.transforms.MelSpectrogram` for magnitude (using `config.n_fft`, `config.hop_length`, `config.n_mels`).
- Create a `torchaudio.transforms.MelScale` for projecting linear-frequency IF into mel domain (using `config.n_fft`, `config.n_mels`).
...
- Step 4: Project IF from linear frequency bins to mel scale. Apply `MelScale` transform to `if_linear` directly: `if_mel = mel_scale(if_linear)`. MelScale is a linear weighted sum across linear frequency bins for each mel bin, so IF values are composited correctly across contributing linear bins. This preserves the IF interpretation as an energy-weighted average IF per mel band.
