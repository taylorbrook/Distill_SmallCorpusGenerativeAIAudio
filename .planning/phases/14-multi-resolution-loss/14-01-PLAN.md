...
must_haves:
  truths:
    - "A LossConfig dataclass exists with nested sub-dataclasses (STFTLossConfig, ReconLossConfig, KLLossConfig) for dot-notation access like config.loss.stft.weight"
    ...
    - "All loss weights are configurable via nested LossConfig without code changes"
  ...
  artifacts:
    ...
    - path: "src/distill/training/config.py"
      provides: "LossConfig dataclass with nested STFTLossConfig, ReconLossConfig, KLLossConfig sub-dataclasses"
      exports: ["LossConfig", "STFTLossConfig", "ReconLossConfig", "KLLossConfig"]
      contains: "class STFTLossConfig"
...
2. In `src/distill/training/config.py`, add nested sub-dataclasses for each loss component, then a parent `LossConfig` that composes them. Place these after `ComplexSpectrogramConfig` and before `TrainingConfig`. This implements the user decision for nested grouped config with dot-notation access (`config.loss.stft.weight`, `config.loss.reconstruction.magnitude_weight`, etc.):
...
Note: Default weights favor STFT loss (1.0) over reconstruction (0.1), per user decision that spectral quality takes precedence. KL defaults match existing config values. The `stft.fft_sizes` use 3 standard resolutions covering different temporal/spectral tradeoffs.

3. Add `loss: LossConfig = field(default_factory=LossConfig)` to `TrainingConfig`. Keep the existing `kl_warmup_fraction`, `kl_weight_max`, and `free_bits` fields on TrainingConfig for backward compatibility but document they are superseded by `loss.kl.*` when the combined loss is used.

Note: `LossConfig` export to `src/distill/training/__init__.py` is handled by 14-02 Task 1 (along with all other __init__ exports for this phase).
  </action>
  <verify>
Run `python -c "from distill.training.config import LossConfig, STFTLossConfig, ReconLossConfig, KLLossConfig, TrainingConfig; c = TrainingConfig(); print(c.loss); print(c.loss.stft.fft_sizes); print(c.loss.reconstruction.magnitude_weight); print(c.loss.kl.weight_max)"` -- should print the LossConfig with nested defaults, FFT sizes tuple (512, 1024, 2048), magnitude_weight 1.0, and kl weight_max 0.01.
Run `python -c "import auraloss; print(auraloss.__version__)"` -- should print version >= 0.4.0.
  </verify>
  <done>LossConfig dataclass exists with nested STFTLossConfig, ReconLossConfig, KLLossConfig sub-dataclasses supporting dot-notation access (config.loss.stft.weight, config.loss.reconstruction.magnitude_weight, config.loss.kl.weight_max). auraloss is installed. TrainingConfig has a `loss` field.</done>
</task>
...
- Compute magnitude-weighted IF loss: weight IF errors by the target magnitude so that errors in low-energy (inaudible) regions contribute less. Use a soft weighting approach:
  ...
  ```
- Combined recon: `loss_config.reconstruction.weight * (loss_config.reconstruction.magnitude_weight * mag_loss + loss_config.reconstruction.if_weight * weighted_if_loss)`
...
- If `_stft_loss_fn` is None, create one from `loss_config.stft.fft_sizes`, `loss_config.stft.hop_sizes`, `loss_config.stft.win_lengths` (lazy initialization pattern).
...
- Weighted: `loss_config.stft.weight * stft_loss`
...
<verification>
1. `python -c "from distill.training.config import LossConfig, STFTLossConfig, ReconLossConfig, KLLossConfig; print(LossConfig())"` prints nested defaults
...
<success_criteria>
- LossConfig dataclass with nested STFTLossConfig, ReconLossConfig, KLLossConfig sub-dataclasses exists in config.py (dot-notation: config.loss.stft.weight, config.loss.reconstruction.magnitude_weight, config.loss.kl.weight_max)
