...
<task type="auto">
  ...
  <files>src/distill/vocoder/__init__.py, src/distill/vocoder/weight_manager.py</files>
  <action>
...
2. **Update `_generate_audio()` handler** to:
   ...
   - If vocoder resolution raises `OSError` (download failure), return error in vocoder_progress output and make retry button visible.
   ...
   - **Important:** Update the return tuple and all early-return error paths to include the new outputs (vocoder_status, vocoder_progress, retry_btn visibility).
...
4. **Update generate_btn.click** to include `vocoder_dropdown` in inputs and new outputs (vocoder_status, vocoder_progress, retry_download_btn).
...
- Status text shows readiness + resolution: "Using: BigVGAN Universal".
  ...
  <done>
    ...
    - Error handling for download failure shows inline error + retry button
