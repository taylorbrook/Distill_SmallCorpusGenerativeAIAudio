...
Provides the complete API for persisting trained VAE models as ``.sda``
...
class LoadedModel:
    ...
    :class:`~small_dataset_audio.inference.generation.GenerationPipeline`
...
def save_model(
    ...
    Creates a ``.sda`` file in *models_dir* and updates the
    :class:`~small_dataset_audio.library.catalog.ModelLibrary` index.
    ...
    models_dir : Path
        Directory to save the ``.sda`` file into.
    analysis : AnalysisResult | None
...
def load_model(
    ...
    """Load a saved ``.sda`` model for immediate generation.
    ...
    ------
    ValueError
        If the file is not a valid ``.sda`` model or has an
        unsupported version.
    """
    import torch  # noqa: WPS433 -- lazy import
...
def delete_model(model_id: str, models_dir: Path) -> bool:
    ...
    models_dir : Path
        Directory containing ``.sda`` files and the library index.
...
def save_model_from_checkpoint(
    ...
    strips optimizer/scheduler state, and saves as a ``.sda`` file
    via :func:`save_model`.
