...
- Default artist tag "SDA Generator" for app-branded provenance.
...
if TYPE_CHECKING:
    from small_dataset_audio.inference.export import ExportFormat
...
def embed_metadata(
    ...
    """
    from small_dataset_audio.inference.export import ExportFormat  # noqa: WPS433
...
def _embed_id3(path: Path, metadata: dict) -> None:
    ...
    # Custom TXXX frames for SDA provenance
    if metadata.get("seed") is not None:
        tags.add(TXXX(encoding=3, desc="SDA_SEED", text=[str(metadata["seed"])]))

    if metadata.get("model_name"):
        tags.add(TXXX(encoding=3, desc="SDA_MODEL", text=[metadata["model_name"]]))

    if metadata.get("preset_name"):
        tags.add(TXXX(encoding=3, desc="SDA_PRESET", text=[metadata["preset_name"]]))
...
def _set_vorbis_tags(audio: object, metadata: dict) -> None:
    ...
    if metadata.get("seed") is not None:
        audio["sda_seed"] = [str(metadata["seed"])]  # type: ignore[index]

    if metadata.get("model_name"):
        audio["sda_model"] = [metadata["model_name"]]  # type: ignore[index]

    if metadata.get("preset_name"):
        audio["sda_preset"] = [metadata["preset_name"]]  # type: ignore[index]
