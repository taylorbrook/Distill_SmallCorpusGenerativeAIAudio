"""CLI entry point for Distill.

Provides the ``distill`` command with subcommands: generate, train, model, ui.
Bare ``distill`` (no subcommand) launches the Gradio GUI for backward
compatibility.
...
app = typer.Typer(
    name="distill",
    help="Distill - Small custom-corpus generative AI audio",
...
def bootstrap(
    ...
    """
    from distill.config.settings import get_config_path, load_config
...
def _main_callback(
    ...
    """Distill - Small custom-corpus generative AI audio."""
...
def _launch_gui(
    *,
    ...
    from distill.app import first_run_setup
...
from distill.cli.ui import app as ui_app

app.add_typer(ui_app, name="ui", help="Launch the Gradio web UI")

from distill.cli.generate import app as generate_app
...
from distill.cli.model import app as model_app
